 ...
 * 
 * Handles all pricing calculations and display updates
 * Implements subscription discounts and site-wide promotions
 */
var PricingManager = exports.PricingManager = /*#__PURE__*/function () {
  function PricingManager() {
    _classCallCheck(this, PricingManager);
    // Pricing constants (in decimal form for calculations)
    this.SUBSCRIPTION_DISCOUNT = 0.25; // 25% off for subscription
    this.SITEWIDE_DISCOUNT = 0.20; // 20% site-wide discount
  ...
  /**
   * Cache pricing display elements
   */
  return _createClass(PricingManager, [{
    ...
    /**
     * Calculate all pricing scenarios for a given base price
    ...
    value: function calculatePrices(basePriceCents) {
      ...
      // Calculate subscription price (25% off base)
      var subscriptionBeforeDiscount = Math.round(basePrice * (1 - this.SUBSCRIPTION_DISCOUNT));

      // Apply site-wide discount to both main and subscription prices
      ...
      return {
        ...
        totalSubscriptionDiscountPercent: (basePrice - finalSubscriptionPrice) / basePrice * 100
      };
    }

    /**
     * Update price display in the UI
     * @param {Object} prices - Calculated prices object
     * @param {string} mode - Current purchase mode ('single' or 'double')
     */
  }, {
    key: "updateDisplay",
    value: function updateDisplay(prices) {
      var mode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'single';
      // Update main (one-time) pricing
      this.updateMainPricing(prices, mode);

      // Update subscription pricing
      this.updateSubscriptionPricing(prices, mode);

      // Update per-serving pricing for double mode
    ...
    /**
     * Update main (one-time purchase) pricing display
     * @param {Object} prices - Calculated prices
     * @param {string} mode - Purchase mode
     */
  }, {
    ...
    value: function updateMainPricing(prices, mode) {
      ...
      if (mainSavings) {
        ...
        if (savingsAmount > 0) {
          mainSavings.querySelector('.savings-amount').textContent = (0, _currency.formatMoney)(savingsAmount);
    ...
    /**
     * Update subscription pricing display
     * @param {Object} prices - Calculated prices
     * @param {string} mode - Purchase mode
     */
  }, {
    ...
    value: function updateSubscriptionPricing(prices, mode) {
      ...
      if (subscriptionSavings) {
        ...
        subscriptionSavings.querySelector('.savings-amount').textContent = (0, _currency.formatMoney)(savingsAmount);
      }
    }
    ...
    value: function updatePriceBreakdown(prices) {
      ...
      if (breakdownSubscription) {
        ...
        breakdownSubscription.textContent = '-' + (0, _currency.formatMoney)(subscriptionDiscount);
      }
      if (breakdownSitewide) {
        var sitewideDiscount = Math.round(prices.subscriptionBeforeDiscount * this.SITEWIDE_DISCOUNT);
        breakdownSitewide.textContent = '-' + (0, _currency.formatMoney)(sitewideDiscount);
      }
      if (breakdownFinal) {
    ...
    /**
     * Calculate price for multiple items (used for double mode)
     * @param {number} basePriceCents - Base price per item
     * @param {number} quantity - Number of items
     * @returns {Object} - Calculated prices for multiple items
     */
  }, {
    ...
    value: function calculateMultipleItemsPricing(basePriceCents) {
      ...
      return _objectSpread(_objectSpread({}, singlePrices), {}, {
        // Override with multiplied amounts
        ...
        // Keep per-item prices for display
     ...
     * @param {number} basePriceCents - Base price to validate
     * @returns {Object} - Validation results and examples
     */
  }, {
    key: "validatePricing",
    value: function validatePricing(basePriceCents) {
      ...
      var expectedSubscriptionBeforeDiscount = Math.round(basePriceCents * 0.75); // 25% off
      var expectedFinalMain = Math.round(basePriceCents * 0.80); // 20% off
      var expectedFinalSubscription = Math.round(basePriceCents * 0.60); // 25% + 20% off
